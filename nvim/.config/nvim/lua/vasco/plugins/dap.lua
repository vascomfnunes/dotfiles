...
function M.config()
  ...
  local daps_path = string.format('%s/mason/packages', vim.fn.stdpath 'data')

  dap_virtual.setup {}

  vim.fn.sign_define(
    'DapBreakpoint',
    { text = icons.breakpoint, texthl = 'LspDiagnosticsSignError', linehl = '', numhl = '' }
  )
  vim.fn.sign_define(
    'DapStopped',
  ...
  vim.fn.sign_define(
    ...
    { text = icons.error, texthl = 'LspDiagnosticSignWarning', linehl = '', numhl = '' }
  )

  -- INSTALATION:
  -- Use Mason or install manually:
  -- cd ~/.local/share/nvim
  ...
  dap.adapters.node2 = {
    type = 'executable',
    command = 'node',
    args = { daps_path .. '/node-debug2-adapter/out/src/nodeDebug.js' },
  }
  ...
  dap.adapters.chrome = {
    type = 'executable',
    command = 'node',
    args = { daps_path .. '/chrome-debug-adapter/out/src/chromeDebug.js' },
  }
